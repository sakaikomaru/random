...
module System.Random.MiddleSquare where
...
newRNG :: IO RNG
newRNG = do
  t1 <- (fromInteger :: Integer -> Word64) <$> getCPUTime
  t2 <- (fromInteger :: Integer -> Word64) <$> getCPUTime
  VU.unsafeThaw $ VU.fromList [t1, t2]

nextWord64 :: RNG -> IO Word64
nextWord64 rng = do
  VUM.unsafeModify rng (^2) 0
  VUM.unsafeModify rng (+s) 1
  w <- VUM.unsafeRead rng 1
  VUM.unsafeModify rng (+w) 0
  x <- VUM.unsafeRead rng 0
  let x1 = (x .>>. 32) .|. (x .<<. 32)
  VUM.unsafeWrite rng 0 x1
  VUM.unsafeRead rng 0
